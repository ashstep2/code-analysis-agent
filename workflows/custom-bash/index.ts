...
import { MODEL_ID, log, TOOL_NAMES, WORKFLOW_CONFIG } from "@/lib/config";
...
export async function customBashWorkflow(
  ...
  const { maxConversationTurns, hookTimeoutMs } = WORKFLOW_CONFIG;
  let turnCount = 0;

  try {
    // Main conversation loop with max iterations guard
    while (turnCount < maxConversationTurns) {
      turnCount++;
      log.workflow(`Turn ${turnCount}/${maxConversationTurns}`);
      ...
      // Wait for user's next message with timeout
      const hookResult = await withTimeout(
        chatHook,
        hookTimeoutMs,
        "Waiting for user message timed out"
      );

      if (!hookResult || hookResult.message === "/done") {
        log.workflow("Conversation ended by user or timeout");
        break;
      }

      // Emit user message to stream (client uses this to split turns)
      await emitUserMessage(hookResult.message);

      // Add to history and continue
      messages.push({ role: "user", content: hookResult.message });
    }

    if (turnCount >= maxConversationTurns) {
      log.workflow("Max conversation turns reached");
    }
  } catch (error) {
    log.error("Workflow", "Error in conversation loop", error);
    throw error;
  } finally {
